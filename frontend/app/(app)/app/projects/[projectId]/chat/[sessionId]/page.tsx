...
export default function ChatPage() {
  ...
  const allSessions = getSessionsByProjectId(projectId);
    ...
    <div className="h-screen flex">
      ...
      <div className="w-[280px] bg-[#faf5e8] border-r border-[#e5e5e5] hidden lg:flex flex-col">
        ...
        <div className="p-4 border-b border-[#e5e5e5] space-y-3">
          ...
          <Link
            href={`/app/projects/${projectId}/chat/new`}
        ...
        <div className="flex-1 overflow-y-auto py-2 space-y-0.5">
          {allSessions.map((s) => (
            <Link
              key={s.id}
              href={`/app/projects/${projectId}/chat/${s.id}`}
              className={cn(
                "flex items-start gap-3 px-4 py-3 mx-2 rounded-xl transition-colors",
                s.id === sessionId ? "bg-[#f5f0e0]" : "hover:bg-[#f5f0e0]/50",
              )}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#9a9a9a] truncate">{s.preview}</p>
              </div>
            </Link>
          ))}
        </div>
